...
from app.config.endpoints import NVD_IMPORT_KEYWORD, NVD_IMPORT_ALL, VULNERABILITIES_BASE
from fastapi import APIRouter, Query, status
from fastapi.responses import JSONResponse
from app.services.nvd import get_cves_by_keyword
from app.services.importer import parse_cves_from_nvd, save_cves_to_db, import_all_cves
from app.database import SessionLocal

router = APIRouter(prefix="/nvd", tags=["nvd"])

@router.post(NVD_IMPORT_KEYWORD, status_code=status.HTTP_202_ACCEPTED)
...
@router.post(NVD_IMPORT_ALL, status_code=status.HTTP_202_ACCEPTED)
